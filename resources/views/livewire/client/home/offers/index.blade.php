                ...
                <div class="swiper-wrapper">
                    @foreach($featuredproducts as $featuredproduct)
                        <a href="/client/pages/product.html" class="swiper-slide text-black">
                            <div>
                                <img src="//product/{{$featuredproduct->id}}/medium/{{$featuredproduct->name}}" alt="slide-img" />
                                <p class="fs-8 fw-bold">{{$featuredproduct->name}}</p>
                            </div>
                            <div class="d-flex align-items-center justify-content-around w-100">
                                <h6>{{$featuredproduct->discount}}</h6>
                                <div class="price mt-3">
                                    <p class="fs-8">{{number_format($featuredproduct->fainalprice)}}<span class="fs-9 me-2">تومان</span></p>
                                    <del class="beforeDiscount fs-9 text-end">{{number_format($featuredproduct->price)}}</del>
                                </div>
                            </div>
                        </a>
                    @endforeach
